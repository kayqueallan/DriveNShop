...
const feedbackService = {
    ...
    createFeedback: async (comentario, tipo_transacao, avaliacao, id_avaliador, id_avaliado) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Erro ao criar feedback: ${response.status} ${response.statusText}`);
            }
    ...
    updateFeedback: async (id, comentario, tipo_transacao, avaliacao, id_avaliador, id_avaliado) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Erro ao atualizar feedback: ${response.status} ${response.statusText}`);
            }
