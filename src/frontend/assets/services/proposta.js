const baseUrl = 'http://localhost:8080/api/proposta';
...
const propostaService = {
    ...
    createProposta: async (proposta) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Erro ao criar proposta: ${response.status} ${response.statusText}`);
            }
    ...
    updateProposta: async (id, proposta) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Erro ao atualizar proposta: ${response.status} ${response.statusText}`);
            }
